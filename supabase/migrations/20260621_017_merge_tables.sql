-- Merge multiple physical tables into one shared order/bill (Phase 2).

alter table orders
  add column if not exists linked_tables text[] not null default '{}';

create index if not exists orders_linked_tables_idx on orders using gin (linked_tables);
...
begin
  ...
  if v_new_table_number = coalesce(v_order.table_number, '') or v_new_table_number = any(v_order.linked_tables) then
  ...
  if exists (
    select 1
     ...
     where status in ('preparing','ready')
       and id <> v_order.id
       and (table_number = v_new_table_number or v_new_table_number = any(linked_tables))
  ) then
...
create or replace function merge_table_into_order(
  p_order_id uuid,
  p_table_number text
)
returns jsonb
...
begin
  ...
  if v_table_number = v_order.table_number then
    raise exception 'Table is already the primary table for this order';
  end if;

  if v_table_number = any(v_order.linked_tables) then
    raise exception 'Table is already merged into this order';
  end if;
  ...
  if exists (
    select 1
     ...
     where status in ('preparing','ready')
       and id <> p_order_id
       and (table_number = v_table_number or v_table_number = any(linked_tables))
  ) then
    raise exception 'Table is already occupied';
  end if;

  update orders
     set linked_tables = array_append(linked_tables, v_table_number)
   where id = p_order_id
...
revoke all on function merge_table_into_order(uuid, text) from public;
grant execute on function merge_table_into_order(uuid, text) to authenticated;

create or replace function unmerge_table_from_order(
  p_order_id uuid,
  p_table_number text
)
returns jsonb
...
begin
  ...
  update orders
     set linked_tables = array_remove(linked_tables, v_table_number)
   where id = p_order_id
     and status in ('preparing','ready')
   returning * into v_updated;
...
revoke all on function unmerge_table_from_order(uuid, text) from public;
grant execute on function unmerge_table_from_order(uuid, text) to authenticated;
