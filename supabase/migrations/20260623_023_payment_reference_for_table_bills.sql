-- Require transaction reference numbers for table GCash and BPI settlements.
...
revoke all on function close_table_bill(text, text, numeric, jsonb, text) from public;
grant execute on function close_table_bill(text, text, numeric, jsonb, text) to authenticated;
