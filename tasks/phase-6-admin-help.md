# Task: phase-6-admin-help

## Goal
Add an admin-only **Help** page at `/admin/help` that renders the Saiko admin manual inline with a left-side table of contents and a Print button. The content is pre-written in this spec as JSX. No new dependencies. No migration.

## Why
Staff need the manual where they actually work. A bookmarked admin page beats a markdown file in a repo. The Print button lets them save a PDF or print the quick reference card to pin near the counter.

## Critical compatibility notes
- Read-only page. No DB calls, no schema, no Edge Function.
- Reuses `AdminLayout` and `AdminGuard` patterns from existing admin pages.
- Source of truth for the content is THIS spec. The standalone `docs/admin-manual.md` file is kept as a separate distributable doc; do NOT delete it.
- Content is delivered as JSX (Option A from planning). No markdown parser is involved.

## Files to modify

### 1. `client/src/App.tsx`
Register the protected route alongside other admin routes:
...
Add the import: `import AdminHelp from "./pages/admin/Help";`

### 2. `client/src/components/AdminLayout.tsx`
Add a **Help** nav item at the BOTTOM of the admin sidebar nav, just above (or in place of) the existing Sign Out item. Use the `BookOpen` icon from lucide-react. Active state when location starts with `/admin/help`. Match the styling of the existing nav items.

If Sign Out is the last item, place Help directly above it.

## Files to create

### 3. `client/src/pages/admin/Help.tsx`
Create this file with the EXACT content below. It is intentionally one self-contained file. Do not refactor sections into separate components.
...
## Files to delete
None. Keep `docs/admin-manual.md` as the standalone printable doc.

## Constraints
Inherits from `AGENTS.md`. Specific:
- **No new npm dependencies.**
- **No migration, no Edge Function, no DB calls.**
- Brand colors only (the inline `<style>` uses brand hex values).
- Do NOT modify any other admin page or component.
- Do NOT change `docs/admin-manual.md`.
- Do NOT split the manual content into separate components or files. Keep it self-contained in `Help.tsx`.

## Reference patterns
- Admin page wrapper: any existing `client/src/pages/admin/*.tsx`
- Sticky sidebar TOC: similar to `client/src/components/CategoryNav.tsx` sticky pattern
- Print styles: `client/src/pages/admin/Dashboard.tsx` (the AI report print rules) and `client/src/pages/admin/PrintSlip.tsx`

## Acceptance criteria
- [ ] `grep -rn "[—–]" client/src` returns nothing
- [ ] `npx tsc --noEmit` passes
- [ ] `client/src/pages/admin/Help.tsx` exists
- [ ] `App.tsx` registers `/admin/help` route protected by `AdminGuard`
- [ ] `grep -n "Help" client/src/components/AdminLayout.tsx` returns at least one match (the new nav item)
- [ ] `grep -n "BookOpen" client/src/components/AdminLayout.tsx` returns at least one match (the icon)
- [ ] `grep -n "@media print" client/src/pages/admin/Help.tsx` returns at least one match
- [ ] `grep -n "Quick reference card" client/src/pages/admin/Help.tsx` returns at least one match
- [ ] `docs/admin-manual.md` still exists and is unchanged

## Out of scope
- Inline search of the manual
- Versioning the manual content
- Auto-extracting the manual from `docs/admin-manual.md` at build time
- Translating the manual to Tagalog
- Embedded videos or screenshots
- Editing the manual via the admin UI

## Notes for Codex
- Copy the Help.tsx content block above EXACTLY. Do not reformat the JSX, do not split into multiple files, do not extract the inline `<style>` into a separate CSS file.
- The inline `<style>` block defines `.manual-content` typography and a print rule. The print rule hides everything in the `admin-print-scope` except the `manual-content-print` article. This matches the Dashboard print pattern.
- Mobile TOC is a collapsible dropdown above the article. Desktop TOC is a sticky aside on the left.
- The `useState` for `tocOpen` only affects mobile. Desktop ignores it.
- The Print button calls `window.print()` directly. The browser handles the PDF save dialog.
- After Codex finishes, override permission applies for THIS session: you ARE allowed to commit and push to origin/master. Use a clear message like `feat(admin): add Help page rendering manual with TOC and print`. Run `npx tsc --noEmit` and the grep acceptance checks BEFORE committing. If any check fails, do NOT commit and report back.
